...
| `substance` | `div.substance` text (lowercased, `/` becomes `, `) |
...
The experience search endpoint. Erowid exposes this as a form submission on
the site. The parameters below were recovered from the form action and the
sort links in the result table.
...
carry no CSS class**, so the parser detects both cases (explicit class first,
...
Rows are `tr.topic-surround` inside `table.topic-chart-surround`. The first
...
| `info` | `<a>` tags inside `div.summary-card-icon-surround`, mapped `{img.alt: href}` | all |
...
All parsing is HTML-structure-dependent. Erowid is a manually maintained
site, so markup can be inconsistent between pages, especially older entries.
Missing `div.title`, absent `table.footdata` rows, or a missing dose chart
are handled gracefully (fields default to empty strings or empty lists). The
ID space is sparse: many IDs in the 1 to 111,451 range return empty pages.
